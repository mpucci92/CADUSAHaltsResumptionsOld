...
def finVizNewsRatings(stock):
    stock = finvizfinance(stock)
    usaNewsDF = stock.TickerNews().head(20)
...
if __name__ == '__main__':
    ...
    with open(html_file,"a") as htmlFile:
        ...
        for j,df in enumerate(resumptions_companies_list):
            ...
            htmlFile.write(df)
            company = resumptiondf.Company.iloc[j]
        ...
        htmlFile.write('<h1>United States Halted/Resumed Tickers</h1>')

        for j,company in enumerate(cleanNamesAmerican(dfUSA)):
            try:
                htmlFile.write(f'<p style="background:orange;padding: 1em;font-weight: bold">Company Name: {company}</p>')
                htmlFile.write(f'<p style="font-size:1vw;font-weight: bold"> Company Website: <a href="https://www.google.ca/search?q={company}"> {company} Website</a> </p>')
                htmlFile.write(f'<p style="font-size:1vw;font-weight: bold"> Manual Search: <a href="https://finviz.com/quote.ashx?t={company}"> {company}</a> </p>')

                ticker = dfUSA.Symbol.iloc[j]
                try:
                    newsdf=finVizNewsRatings(ticker)
                    newsdf = newsdf.to_html()
                    htmlFile.write(newsdf)
                    htmlFile.write('<br>')
